...
func init() {
	// Initialize global agent registry for tests
	messenger := NewTerminalMessenger(os.Stderr)
...
func TestBox_EffectiveMounts(t *testing.T) {
	tests := []struct {
		...
		box      *Box
		wantLen  int
		validate func(t *testing.T, mounts []sandtypes.MountSpec)
	}{
		{
			name: "custom mounts",
			box: &Box{
...
func TestBox_CreateContainer(t *testing.T) {
	...
	t.Run("success with default mounts", func(t *testing.T) {
		...
		box := &Box{
			ID:        "test-box",
			AgentType: "default", SandboxWorkDir: tmpDir,
...
func TestBox_executeHooks(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("multiple hooks success", func(t *testing.T) {
		...
		hook2 := sandtypes.NewContainerStartupHook("hook2", func(ctx context.Context, box sandtypes.BoxOperations) error {
	...
	t.Run("partial hook failure", func(t *testing.T) {
		...
		hook3 := sandtypes.NewContainerStartupHook("hook3", func(ctx context.Context, box sandtypes.BoxOperations) error {
			successCount++
			return nil
		})
