...
func TestDefinitionContainerConfigurationAddsInstallHookAfterBaseHook(t *testing.T) {
	...
	wantCalls := []string{
		...
		"exec:test -f /usr/local/share/ca-certificates/sand-http-cache.crt",
		"stream:env PATH=/opt/sand-agent-cache/node/22.23.1/linux-x64/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin NODE_EXTRA_CA_CERTS=/usr/local/share/ca-certificates/sand-http-cache.crt /opt/sand-agent-cache/node/22.23.1/linux-x64/bin/npm install -g --prefix /usr/local/lib/sand-npm-agents/codex @openai/codex@0.145.0",
		"exec:head -c 4 /usr/local/lib/sand-npm-agents/codex/bin/codex",
